...
static bool CreateDeviceD3D12(HWND hWnd)
{
    ...
    if (CreateDXGIFactory1(IID_PPV_ARGS(&g_dxgiFactory)) != S_OK) return false;
...
static std::add_pointer_t<
  HRESULT WINAPI(IDXGISwapChain3 *, UINT, UINT, UINT, DXGI_FORMAT, UINT, const UINT *, IUnknown *const *)>
  oResizeBuffers1;
...
{
    if (!g_pd3dCommandQueue) { g_pd3dCommandQueue = pCommandQueue; }
...
static std::add_pointer_t<HRESULT
    WINAPI(IDXGIFactory *, IUnknown *, IUnknown *, const DXGI_SWAP_CHAIN_DESC1 *, IDXGIOutput *, IDXGISwapChain1 **)>
...
static std::add_pointer_t<
  HRESULT WINAPI(IDXGIFactory *, IUnknown *, const DXGI_SWAP_CHAIN_DESC1 *, IDXGIOutput *, IDXGISwapChain1 **)>
...
void Hook(HWND hwnd)
{
    ...
    if (!CreateDeviceD3D12(GetConsoleWindow())) {
        console::log("CreateDeviceD3D12() failed");
        return;
    }
    ...
    if (g_pd3dDevice) {
        ...
        static MH_STATUS resize1Status = MH_CreateHook(
          reinterpret_cast<void **>(fnResizeBuffers1), &hkResizeBuffers1, reinterpret_cast<void **>(&oResizeBuffers1));
...
static void RenderImGui_DX12(IDXGISwapChain3 *pSwapChain)
{
        ...
        if (SUCCEEDED(pSwapChain->GetDevice(IID_PPV_ARGS(&g_pd3dDevice)))) {
            ...
            for (UINT i = 0; i < NUM_BACK_BUFFERS; ++i)
                if (g_pd3dDevice->CreateCommandAllocator(
                      D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&g_commandAllocators[i]))
                    != S_OK)
                    return;
    ...
    // not shutting down
    if (true) {
        if (!g_mainRenderTargetResource[0]) { CreateRenderTarget(pSwapChain); }
        ...
        if (ImGui::GetCurrentContext() && g_pd3dCommandQueue && g_mainRenderTargetResource[0]) {
            ...
            g_pd3dCommandQueue->ExecuteCommandLists(
              1, reinterpret_cast<ID3D12CommandList *const *>(&g_pd3dCommandList));
        }
    }
}