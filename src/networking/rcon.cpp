		...
		while (true) {
			...
			if (recv_bytes > 10){
				...
				if (std::memcmp(static_cast<char*>(this->buffer) + strlen(rcon_command), password.c_str(), password.length()) != 0) {
                    ...
                    const char *rcon_invaid_password = "\xFF\xFF\xFF\xFFRCONINVALIDPASSWORD";
					sendto(this->sock, rcon_invaid_password, strlen(rcon_invaid_password), 0, (struct sockaddr *)&client_addr, addrlen);

					continue;
				}

				auto command_bytes = static_cast<char *>(this->buffer) + strlen(rcon_command) + password.length();
                std::string cmd(command_bytes, 0, recv_bytes - strlen(rcon_command) - password.length()-1);
